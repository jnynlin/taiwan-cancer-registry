...
\hypersetup{
  ...
  pdftitle={Surgical Margin Status, CCRT, and Deep Learning-Identified Patient
    Subtypes as Independent Prognostic Determinants in Esophageal Cancer}
}
...
\begin{center}
{\LARGE\bfseries Surgical Margin Status, CCRT, and Deep
Learning--Identified Patient Subtypes as Independent Prognostic
Determinants in Esophageal Cancer\par}
...
and multivariable Cox proportional hazards regression (apparent C-index
0.705; bootstrap out-of-bag C-index 0.703, 95\,\%\,CI 0.687--0.720,
$B = 1000$) quantified independent
...
deaths (77.4\,\%) and median OS of 13.4 months. R0 resection margin
(HR 0.60, 95\,\%\,CI 0.53--0.69) and concurrent chemoradiotherapy
(CCRT; HR 0.76, 0.67--0.85) were the strongest independent modifiable
prognostic determinants ($p < 0.001$; apparent C-index 0.705,
bootstrap out-of-bag C-index 0.703, 95\,\%\,CI 0.687--0.720, $B = 1000$).
Endoscopic resection (HR 0.47) and radical esophagectomy (HR 0.61) also
conferred independent benefit, though endoscopic resection reflects
...
\textbf{Conclusions:} R0 margin and CCRT are primary therapeutic targets
in esophageal cancer. Unsupervised deep learning reveals clinically
meaningful patient heterogeneity beyond conventional staging.
...
limitation (Section~\ref{sec:limitations}). The final Cox cohort
comprised 2,364 patients (including 609 with imputed stage indicators).
The concordance (C-) index was computed from the fitted model; a
bootstrap 95\,\% confidence interval was derived from $B = 1000$
resamples by re-fitting the model on each bootstrap sample and evaluating
on the held-out cases. Hazard ratios (HRs) with 95\,\% confidence
intervals (CI) are reported. Reference categories were: Stage I (or
unknown), no surgery, R1/R2 margin, no CCRT, and single-agent or no
chemotherapy.
...
a finding explained by confounding by indication: patients with more
advanced or metastatic disease are less likely to be prescribed CCRT,
creating a selection bias that suppresses the crude CCRT benefit. After
multivariable adjustment for stage, surgical modality, and margin status
in the Cox model, CCRT independently reduced hazard by 24\,\% (HR 0.76,
95\,\% CI 0.67--0.85, $p < 0.001$; Table~\ref{tab:cox}). This pattern
is well-recognised in registry-based chemotherapy analyses and reinforces
the necessity of adjusted rather than unadjusted comparisons.

\subsection{Multivariable Cox proportional hazards analysis}
Multivariable Cox regression was fitted to 2,364 patients
(Figure~\ref{fig:cox}; Methods §\ref{sec:cox} for stage imputation
details). The apparent C-index was 0.705; the bootstrap out-of-bag
C-index was 0.703 (95\,\% CI 0.687--0.720, $B = 1000$), indicating
negligible optimism ($< 0.002$) attributable to ridge penalisation.
Independent prognostic predictors are summarised in Table~\ref{tab:cox}.
After adjusting for all covariates, R0 resection margin was the single
strongest modifiable predictor reflecting surgical quality (HR 0.60,
95\,\% CI 0.53--0.69, $p < 0.001$), followed by CCRT (HR 0.76,
0.67--0.85, $p < 0.001$). Endoscopic resection showed the largest
HR (0.47, 0.38--0.59, $p < 0.001$), but this reflects
selection of early-stage disease amenable to endoscopic treatment rather
than a modifiable surgical quality metric. Radical esophagectomy
independently reduced hazard by approximately 40\,\% (HR 0.61, 0.51--0.72,
$p < 0.001$). Advanced stage (III and IV vs I) and male sex were adverse
prognostic factors.

A sensitivity analysis restricted to the 1,757 patients with known AJCC
stage yielded consistent results: R0 margin HR 0.59, CCRT HR 0.72,
radical esophagectomy HR 0.58 (all $p < 0.001$; C-index 0.724),
confirming that the stage-imputation approach does not materially
distort the primary estimates.
...
identified surgery, AJCC stage, and CCRT as the three most discriminating
...
\subsection{CCRT confers independent survival benefit}
CCRT independently reduced hazard by 24\,\% (HR 0.76, 95\,\% CI
0.67--0.85, $p < 0.001$) after adjustment for surgical modality and
margin status, supporting its role as both definitive non-surgical
therapy (for unresectable cases) and perioperative treatment augmentation.
The crude regimen-level comparisons are subject to selection bias by
indication: patients receiving multi-agent and targeted regimens had more
advanced disease at baseline (Stage IV enrichment), explaining the lower
crude OS in these groups despite likely biological benefit in appropriately
selected patients. This selection bias is a well-recognised limitation of
registry-based chemotherapy analyses and should be interpreted accordingly
\cite{Lordick2022}.

Cumulative cycle data were available in only 5.3\,\% of patients,
precluding robust dose-response analysis. Prospective registration of
cycle counts and drug identifiers (e.g., cisplatin vs. carboplatin
backbone) is a priority for future registry data collection. The absence
of drug-level data also prevents direct comparison with the NEO-AEGIS
trial or KEYNOTE-590 protocols \cite{Shapiro2021, Sun2021}.
...
DeepSurv feature importance confirmed surgery, stage, and CCRT as the
dominant discriminators --- as expected, since DeepSurv was trained on the
same cohort and features as the Cox model (internal consistency, not
...
Second, AJCC stage was unknown in 609 patients (25.7\,\%). Rather than
performing listwise deletion, the Cox model coded stage-unknown patients as
zero on all stage indicator variables (effectively grouping them with the
Stage I reference). This approach retains cases at the cost of introducing
potential misclassification for the stage coefficient estimates; multiple
imputation by chained equations would be the preferred approach and is a
...
a composite loss function separating continuous from categorical features
would provide more principled latent-space geometry and is recommended for
future work.
...
A model adding cluster assignment to histology and subsite would test
whether clustering explains additional OS variance.
...
In this comprehensive registry-based analysis of 2,367 esophageal cancer
patients in Taiwan, R0 resection margin attainment and CCRT are
independently the strongest modifiable prognostic determinants of overall
survival. Surgical quality --- reflected by R0 status and the extent of
lymph node dissection --- deserves emphasis equal to the decision to
operate. Unsupervised deep learning recovers three patient subtypes
--- adenocarcinoma-enriched, mainstream SCC, and cervical/upper esophagus
--- consistent with known histological and anatomical subgroups;
incremental Cox analysis confirms these clusters do not explain OS
variance beyond histology and subsite coding ($\Delta$C-index $= -0.001$).
The principal value of the autoencoder lies in demonstrating that
data-driven grouping organically recapitulates clinically recognised
subtypes, validating the internal coherence of the latent space.
These findings support a research priority of enhancing surgical quality
metrics within the Taiwan Cancer Registry and prospective molecular
characterisation of the adenocarcinoma-enriched subgroup.
...
\begin{table}[H]
  ...
  \caption{Multivariable Cox proportional hazards model results
    ($n = 2{,}364$; apparent C-index 0.705; bootstrap out-of-bag
    C-index 0.703, 95\,\%\,CI 0.687--0.720, $B = 1000$).
    Stage-unknown patients ($n = 609$) coded zero on all stage
    indicators (grouped with Stage~I reference).}
  \label{tab:cox}
  \begin{tabular}{lrrl}
    \toprule
    ...
    Male sex                    & 1.18 & 0.92--1.53  & 0.195 \\
    Stage II vs I               & 1.45 & 1.10--1.91  & 0.008 \\
    Stage III vs I              & 2.01 & 1.55--2.60  & $< 0.001$ \\
    Stage IV vs I               & 3.44 & 2.65--4.47  & $< 0.001$ \\
    Endoscopic resection        & 0.47 & 0.38--0.59  & $< 0.001$ \\
    Radical esophagectomy       & 0.61 & 0.51--0.72  & $< 0.001$ \\
    R0 resection margin         & 0.60 & 0.53--0.69  & $< 0.001$ \\
    CCRT                        & 0.76 & 0.67--0.85  & $< 0.001$ \\
    Multi-agent chemotherapy    & 0.91 & 0.80--1.04  & 0.163 \\
    \bottomrule
    \multicolumn{4}{p{0.70\textwidth}}{\small HR = hazard ratio; CI =
      confidence interval. Reference categories: Stage I (or unknown);
      no surgery; R1/R2 margin; no CCRT; single-agent or no chemotherapy.
      Ridge penaliser = 0.1. Stage-unknown patients ($n = 609$) coded
      zero on all stage dummies and retained in analysis.}
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{\textbf{Overall survival by CCRT status (unadjusted Kaplan-Meier).}
    ...
    (log-rank $p = 0.58$, non-significant). The absence of crude survival
    difference reflects confounding by indication: patients with more
    advanced or metastatic disease are under-represented in the CCRT group.
    After multivariable adjustment for stage, surgical modality, and margin
    status, CCRT independently reduced mortality risk (HR 0.76,
    95\,\% CI 0.67--0.85, $p < 0.001$; Table~\ref{tab:cox}).}
  \label{fig:chemo}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{\textbf{Multivariable Cox proportional hazards forest plot ---
    ...
    ($n = 2{,}364$; apparent C-index 0.705; bootstrap OOB C-index 0.703,
    95\,\% CI 0.687--0.720, $B = 1000$). Covariates adjusted for age,
    sex, and AJCC stage.
