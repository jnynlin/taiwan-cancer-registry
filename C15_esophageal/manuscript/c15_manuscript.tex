...
(HR 0.63, 95\,\%\,CI 0.55--0.73, $p < 0.001$). CCRT was not
independently significant under MICE imputation (HR 0.97, $p = 0.569$),
consistent with the unadjusted Kaplan-Meier result ($p = 0.58$); a
sensitivity analysis with zero-coded stage yielded HR 0.76 ($p < 0.001$),
likely artefactual due to stage misclassification. Radical
esophagectomy (HR 0.61) also conferred significant independent benefit.
...
benefit is sensitive to stage imputation method and was non-significant
under MICE; this uncertainty warrants prospective clarification.
...
1000$ resamples. Hazard ratios (HRs) with 95\,\% confidence intervals
(CI) are reported. Reference categories were: Stage I, no surgery, R1/R2
margin, no CCRT, and single-agent or no chemotherapy.
...
This finding does not imply CCRT lacks biological efficacy in esophageal
cancer --- the CROSS trial demonstrated clear benefit in resectable disease
\cite{vanHagen2012}. Rather, it illustrates a well-recognised limitation
of registry-based CCRT analyses: confounding by indication and stage
misclassification can produce spurious adjusted HRs. The MICE result
should be interpreted as the more credible estimate in this registry
dataset.
...
a sensitivity. The divergence in the CCRT estimate between MICE
(HR 0.97, NS) and zero-coded (HR 0.76, $p < 0.001$) confirms that the
zero-coded approach introduced misclassification bias into the CCRT
coefficient.
...
benefit was non-significant under the primary MICE imputation analysis
(HR 0.97, $p = 0.569$), converging with the unadjusted Kaplan-Meier
result; the previously reported zero-coded HR (0.76, $p < 0.001$) was
likely artefactual due to stage misclassification bias and should not be
used to guide treatment allocation.
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrl}
    ...
    \multicolumn{4}{p{0.75\textwidth}}{\small HR = hazard ratio; CI =
      ...
      $^\ddagger$CCRT HR 0.76 ($p < 0.001$) in zero-coded stage sensitivity
      analysis; the MICE estimate ($p = 0.569$) is the pre-specified
      primary result.}
  \end{tabular}
\end{table}
